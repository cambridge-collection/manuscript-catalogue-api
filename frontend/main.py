...
def translate_params(resource_type: str, **url_params):
    ...
    date_min = generate_datestring(
        get_obj_property("year", set_params),
        get_obj_property("month", set_params),
        get_obj_property("day", set_params),
    )
    date_max = generate_datestring(
        get_obj_property("year-max", set_params),
                ...
                for date in val_list:
                    ...
                    for index, field in enumerate(fields):
                        if (num_parts - 1) <= index:
                            filter["f.%s.facet.contains" % field] = re.sub(
                                r'^"(.+?)"$', r"\1", date
                            )
                        else:
                    ...
                    fq.append(
                        '%s:"%s"' % (solr_name, re.sub(r'^"(.+?)"$', r"\1", date))
                    )
...
def generate_datestring(year, month, day):
    ...
    if year:
        ...
        if not (day and not month):
            valid_tokens = [
                str(i).zfill(2) for i in [year, month, day] if i is not None
            ]
...
async def get_request(resource_type: str, **kwargs):
    ...
    try:
        ...
        if hasattr(e.response, "text"):
            ...
            raise HTTPException(
                status_code=results["responseHeader"]["status"],
                detail=results["error"]["msg"],
            )
        else:
...
@app.get("/summary")
async def get_summary(q: List[str] = Query(default=None), fq: Union[str, None] = None):
    q_final = " AND ".join(q) if hasattr(q, "__iter__") else q
...
async def update_collection(request: Request):
    ...
    if json_dict["name"]:
        ...
        status_code = await put_item(
            "collection", data, {"f": ["$FQN:/**", "id:/name/url-slug"]}
        )
    else:
